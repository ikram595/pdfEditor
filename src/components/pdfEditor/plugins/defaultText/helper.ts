...
export const widthOfTextAtSize = (
  ...
  const standardWidth =
    glyphs.reduce(
      (totalWidth, glyph) => totalWidth + glyph.advanceWidth * scale,
      0
    ) *
    (fontSize / 1000);
...
export const getFontKitFont = async (
  ...
  const currentFont =
    font[fntNm] || getFallbackFont(font) || getDefaultFont()[DEFAULT_FONT_NAME];
...
export const calculateDynamicFontSize = async ({
  ...
  const shouldFontGrowToFit = (
    totalWidthInMm: number,
    totalHeightInMm: number
  ) => {
  ...
  let { totalWidthInMm, totalHeightInMm } =
  ...
  while (shouldFontShrinkToFit(totalWidthInMm, totalHeightInMm)) {
    dynamicFontSize -= FONT_SIZE_ADJUSTMENT;
    ({ totalWidthInMm, totalHeightInMm } =
      calculateConstraints(dynamicFontSize));
  }
